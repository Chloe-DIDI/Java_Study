...
○ String 클래스 객체 간에 『+』연산자를 사용하게 되면
   ...
   ※ 눈으로 확인되는 결과는 이렇게 되지만...
	  String 은 내용을 변경 할 수 없기 때문에
	  『+』연산자를 사용하게 되면
	  내부적으로 StringBuffer 를 생성하여
	  append() 메소드를 이용하여
	  문자열에 대한 결합을 수행하게 된다.
	  따라서, 도적인 문자열을 사용하는 Servlet 등에서는
...
public class Test132
{
	...
	{	
		...
		//	 따라서 『s3 == s4』는 거짓이 되며
		//	 만약, 같은 문자열 상수인지의여부를 비교해야 할 경우라면
		//	 『equals』메소드를 이용해야 한다.
		...
		System.out.println("s1 : " + s1.haseCode());
		System.out.println("s2 : " + s2.haseCode());
		System.out.println("s3 : " + s3.haseCode());
		System.out.println("s4 : " + s4.haseCode());
