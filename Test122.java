...
○ 개요
	...
	이러한 클래스나 인터페이스의 묶ㅇㅁ을 패키지(package)라고 한다.
...
○ import 구문
	...
	import [static] 패키지.클래스;	//[] 안에있는건 생략가능
	import [static] 패키지.*;		//[] 안에있는건 생략가능

	※ JDK 1.5 부터는 import 다음네 『static』 키워드를 붙임으로써
	정적으로 패키지를 import 할 수 있는 기능을 제공하게 되었다.
	만약 패키지를 정적으로 import 한 경우라면 모든 접근 가능한
	메소드와 멤버 변수들은 접두사(ex → ob, br, sc 처럼..._)을 붙이지 않고
	사용하는 것이 가능하다.

○ Object 클래스
	『java.lang.Object』클래스는 자바 클래스에[서 최상위 클래스로
	자바 표준 패키지의 클래스나 사용자에 의해 작성된 모~~~~~~~~~든 클래스는
	기본적으로 이 클래스로부터 상속받는다.
	따라서 자바의 모든 클래스는 클래스의 『java.lang.Object』
	메소드를 가지고있으며, 바로 사용하는 것이 가능하다.
